...
SettingsWidget::SettingsWidget(StippleViewer *stippleViewer, QWidget *parent)
  ...
  spinInitialPointSize->setToolTip(
      "The point size used by the algorithm. This "
  ...
  spinMaxIter->setToolTip(
      "The maximum number of iterations until the "
  ...
  connect(m_savePDF, &QPushButton::pressed, [this]() {
    QFileDialog dialog(this, tr("Save Image as PDF"), QString(),
                       tr("PDF (*.pdf)"));
    dialog.setAcceptMode(QFileDialog::AcceptSave);
  ...
  connect(m_stippleViewer, &StippleViewer::finished, this,
          &SettingsWidget::enableSaveButtons);
